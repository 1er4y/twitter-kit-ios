...
- (void)showActionSheetWithCompletion:(TWTRAuthenticationProviderCompletion)completion
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [[TWTRUtils topViewController] presentViewController:alertController animated:YES completion:NULL];
    });
}
...
- (UIAlertController *)alertController
{
    ...
    [self.accounts enumerateObjectsUsingBlock:^(ACAccount * _Nonnull account, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        UIAlertAction *alertAction = [UIAlertAction actionWithTitle:title style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            TWTRAuthenticationProviderCompletion completion = objc_getAssociatedObject(controller, (__bridge const void *)(TWTRSocialAppProviderActionSheetCompletionKey));
            [self getAuthTokenWithAccount:[self accounts][idx] completion:completion];
        }];
